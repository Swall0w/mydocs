...
指定された引数の数をチェックする
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
引数の指定を必要とするシェルスクリプトを正常に実行するには、実行時に必要とする引数の数が正確に指定されているかチェックする必要がある。
これは、実行に引数が必要とするかだけではなく、引数無しで実行された場合にシェルスクリプトが予期せぬ動作をすることを防ぐためでもある。

この引数が必要な数だけ指定されたかどうかは、 **$#** を参照することで確認できる。
字祭にこの引数を利用した例を以下に示す。このシェルスクリプトは実行に3つの引数を必要として、引数が正確に3つ指定されていない場合にはエラー終了する。 ::

    #!/bin/bash

    #   実行時に変数 **$#** の値が3つでなければ、エラー終了
    if [ $# -ne 3 ]; then
...
Usageメッセージを表示する。
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
**$0** を用いて、Usageメッセージを表示するシェルスクリプトを作成する。このシェルスクリプトは引数を2つ必要とし、
実行時の日付と時刻を引数に指定したファイルにそれぞれ保存する。 ::
...
**$0** はパスを含んでいる可能性があるため、basenameコマンドを用いて、自分自身のファイル名のみを取得する。$0が"./usage.sh"だった場合には、basenameコマンドの
結果は"usage.sh"となる。
